...
public static class GuardHelper
{
    ...
    )
    {
        ...
        if (value != expectedValue)
        {
            throw new ArgumentOutOfRangeException(
                parameterName,
                value,
                description ?? $"Parameter must be {expectedValue}."
            );
        }
    ...
    public static T Same<T>(string parameterName, T expected, T actual)
    {
        ...
        if (!expected2.Equals(actual))
        {
            throw new ArgumentException(
                $"Parameter must be {expected2}. Actual: {actual}.",
                parameterName
            );
        }
    ...
    public static T MinimumAndNotNull<T>(string parameterName, T value, T smallest)
        where T : IComparable
    {
        NotNull(parameterName, value);
        ...
        if (value.CompareTo(smallest) < 0)
        {
            throw new ArgumentOutOfRangeException(
                parameterName,
                value,
                $"Parameter cannot be less than {smallest}."
            );
        }
    ...
    )
    {
        ...
        if (count < minCount || count > maxCount)
        {
            throw new ArgumentOutOfRangeException(
                containerName,
                count,
                $"{containerName}.Count() cannot be less than {minCount} or greater than {maxCount}."
            );
        }
        return container;
    }

    public static T InRangeAndNotNull<T>(string parameterName, T value, T smallest, T greatest)
        where T : IComparable
    {
        NotNull(parameterName, value);
        ...
        if (value.CompareTo(greatest) > 0)
        {
            throw new ArgumentOutOfRangeException(
                parameterName,
                value,
                $"Parameter cannot be greater than {greatest}."
            );
        }
