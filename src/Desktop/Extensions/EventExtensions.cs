...
public static class EventExtensions
{
    ...
    )
    {
        ...
        var eventInfo =
            value.GetType().GetEvent(eventName)
    ...
    /// <exception cref="InvalidOperationException"></exception>
    /// <returns></returns>
    public static async Task<Dictionary<string, T>> WaitAllEventsAsync<T>(
        this object value,
        Func<CancellationToken, Task> func,
        CancellationToken cancellationToken = default,
    ...
    )
    {
        ...
        var tasks = eventNames
            .Select(async name =>
            {
                ...
                {
                    return await value
                        .WaitEventAsync<T>(name, cancellationToken)
    ...
    /// <exception cref="InvalidOperationException"></exception>
    /// <returns></returns>
    public static async Task<Dictionary<string, T>> WaitAnyEventAsync<T>(
        this object value,
        Func<CancellationToken, Task> func,
        CancellationToken cancellationToken = default,
        params string[] eventNames
