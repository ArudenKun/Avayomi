...
public class MegaUpExtractor : VideoExtractorBase
{
    private const string ApiBase = "https://enc-dec.app/api";

    public MegaUpExtractor(IHttpClientFactory httpClientFactory)
        : base(httpClientFactory) { }

    public override string ServerName => "MegaUp";
    ...
    )
    {
        ...
        var userAgent = HttpHelper.ChromeUserAgent();
        ...
        var http = HttpClientFactory.CreateProviderHttpClient();
        var response = await http.ExecuteAsync(mediaRequest, cancellationToken);
        ...
        var decResponse = await http.ExecuteAsync(decRequest, cancellationToken);
    ...
    )
    {
        ...
        var http = HttpClientFactory.CreateProviderHttpClient();
        var response = await http.ExecuteAsync(request, cancellationToken);
        var json = JsonNode.Parse(response);
