...
public static partial class JavaScriptUnpacker
{
    /// <summary>
    /// Regex to detect packed functions.
    /// </summary>
    private static readonly Regex PackedRegex = MyRegex();

    /// <summary>
    /// Regex to get and group the packed javascript.
    /// Needed to get information and unpack the code.
    /// </summary>
    private static readonly Regex PackedExtractRegex = MyRegex1();

    /// <summary>
    /// Matches function names and variables to de-obfuscate the code.
    /// </summary>
    private static readonly Regex UnpackReplaceRegex = MyRegex2();
    ...
    public static bool IsPacked(string? scriptBlock)
    {
        ...
        return PackedRegex.IsMatch(scriptBlock);
    }
    ...
    public static List<string?> GetPackedScripts(IEnumerable<string?> scriptBlocks)
    {
        ...
        foreach (var script in scriptBlocks)
        {
            ...
            if (PackedRegex.IsMatch(script))
            {
                list.Add(PackedRegex.Match(script).Value);
            }
        }
    ...
    private static List<string?> Unpacking(string? scriptBlock)
    {
        ...
        var matches = PackedExtractRegex.Matches(scriptBlock).OfType<Match>();
        ...
        foreach (var match in matches)
        {
            ...
            if (symtab.IsNullOrEmpty() || count is null || symtab.Length != count)
            {
                list.Add(null);
                continue;
            }
            ...
            {
                payload = UnpackReplaceRegex.Replace(
                    payload,
                    eval =>
                    {
                        var word = eval.Value;
    ...
    [GeneratedRegex(@"\b\w+\b", RegexOptions.IgnoreCase | RegexOptions.Multiline, "en-US")]
