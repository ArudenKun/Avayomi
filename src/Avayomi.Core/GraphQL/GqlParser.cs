...
internal static class GqlParser
{
    ...
    public static IList<GqlSelection> ParseToSelections(Type type)
    {
        ...
        foreach (var variable in variables)
        {
            ...
            if (selectionAttribute is null)
                continue;
            var subSelections = ParseToSelections(
                variable.MemberType switch
                {
            ...
            var parameters = variable
                .GetCustomAttributes<GqlParameterAttribute>()
    ...
    {
        return ParseToString([selection]);
    }
    ...
    {
        return node.Deserialize<TObject>(JsonSerializerOptions);
    }
    ...
    private static string BuildParameters(IEnumerable<GqlParameter> parameters)
    {
        ...
        foreach (var parameter in parameters)
            stringBuilder.Append(
                (stringBuilder.Length > 1 ? "," : string.Empty)
                    + parameter.Name
                    + ":"
        ...
        return value switch
            {
                ...
                IEnumerable<GqlParameter> parameters => (
                    (Func<string>)(
                        () =>
                        {
                            var stringBuilder = new StringBuilder();
