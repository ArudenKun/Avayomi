...
public class NineAnime : AnimeBaseProvider, IAnimeProvider, IPopularProvider, ILastUpdatedProvider
{
    ...
    private List<IAnimeInfo> ParseAnimeResponse(string? response)
    {
        ...
        foreach (var node in nodes)
        {
            var animeInfo = new AnimeInfo
            {
                ...
                Id = node.SelectSingleNode(".//div/div[contains(@class, 'ani')]/a")!
                    .Attributes["href"]
                    .Value,
                Title = node.SelectSingleNode(
                    ".//div/div[contains(@class, 'info')]/div[contains(@class, 'b1')]/a"
                )!.InnerText,
                Image = node.SelectSingleNode(".//div/div[contains(@class, 'ani')]/a/img")
                    .Attributes["src"]
                    .Value,
            };
    ...
    private List<IAnimeInfo> ParseAnimeSearchResponse(string? response)
    {
        ...
        foreach (var node in nodes)
        {
            ...
            {
                Site = AnimeSites.Aniwave,
                Id = node.GetAttributeValue("href", ""),
                Title =
                    node.SelectSingleNode(".//div[@class='name d-title']")?.InnerText.Trim()
                    ?? string.Empty,
                Image = node.SelectSingleNode(".//img")?.GetAttributeValue("src", ""),
                Released = node.SelectSingleNode(".//div[@class='meta']/span[last()]")
                    .InnerText.Trim(),
                Type = node.SelectSingleNode(".//div[@class='meta']/span[last()-1]")
                    .InnerText.Trim(),
            };
