...
using System.Text;
using Avayomi.Mpv.Exceptions;
using Avayomi.Mpv.Interop;
using Avayomi.Mpv.Native;

namespace Avayomi.Mpv.Player;

public unsafe class MpvClient : IDisposable
{
    private readonly Lock _gate = new();
    private readonly MpvClientOptions _options;
    ...
    public MpvClient()
        : this(new MpvClientOptions()) { }

    public MpvClient(Action<MpvClient> beforeInitialize)
        : this(new MpvClientOptions { BeforeInitialize = beforeInitialize }) { }

    public MpvClient(MpvClientOptions options)
    {
        _options = options ?? new MpvClientOptions();
        ...
        InitializeCore(_options.SharedClient == null);
    }

    public IntPtr Handle => (IntPtr)_handle;
    public MpvClientOptions Options => _options;
    ...
    public void ObserveProperty(string name, MpvFormat format)
    {
        lock (_gate)
        {
            EnsureActiveHandle();
            ThrowIfError(
                MpvNativeApi.ObserveProperty(_handle, 0, name, format),
                nameof(MpvNativeApi.ObserveProperty),
                name,
                format.ToString()
            );
        }
    ...
    public void SetProperty(string name, long value)
    {
        lock (_gate)
        {
            ...
            long local = value;
            ThrowIfError(
                MpvNativeApi.SetProperty(_handle, name, MpvFormat.Int64, &local),
                nameof(MpvNativeApi.SetProperty),
                name,
                value.ToString()
            );
        }
    ...
    public void SetProperty(string name, string? value)
    {
        lock (_gate)
        {
            EnsureActiveHandle();
            ThrowIfError(
                MpvNativeApi.SetPropertyString(_handle, name, value),
                nameof(MpvNativeApi.SetPropertyString),
                name,
                value ?? "<null>"
            );
        }
    ...
    public void SetProperty(string name, bool value)
    {
        lock (_gate)
        {
            ...
            int local = value ? 1 : 0;
            ThrowIfError(
                MpvNativeApi.SetProperty(_handle, name, MpvFormat.Flag, &local),
                nameof(MpvNativeApi.SetProperty),
                name,
                value.ToString()
            );
    ...
    public double GetDoubleProperty(string name)
    {
        lock (_gate)
        {
            ...
            double value = 0;
            ThrowIfError(
                MpvNativeApi.GetProperty(_handle, name, MpvFormat.Double, &value),
                nameof(MpvNativeApi.GetProperty),
                name
            );
            return value;
        }
    ...
    public void RunCommand(params string[] args)
    {
        lock (_gate)
        {
            ...
            ThrowIfError(
                MpvNativeApi.Command(_handle, nativeArgs.Pointer),
                nameof(MpvNativeApi.Command),
                args
            );
        }
    }
    ...
    public Task RunCommandAsync(string[] args, CancellationToken cancellation = default)
    {
        ...
        try
        {
            lock (_gate)
            {
                ...
                using var nativeArgs = CommandArgumentBlock.Create(args);
                ThrowIfError(
                    MpvNativeApi.CommandAsync(_handle, requestId, nativeArgs.Pointer),
                    nameof(MpvNativeApi.CommandAsync),
                    args
                );
            }
        }
        ...
        return completion.Task.ContinueWith(
            ...
            {
                cancellationRegistration.Dispose();
                _pendingAsyncWork.TryRemove(requestId, out var _);
            },
    ...
    public void EnsureRenderContext()
    {
        ...
        var sharedPlayer = _options.SharedClient;
        ...
        try
        {
            ...
            var parameters = new[]
            {
                new MpvRenderParameter
                {
                    Type = MpvRenderParameterType.ApiType,
                    Data = (void*)apiTypePtr,
                },
                new MpvRenderParameter
                {
                    Type = MpvRenderParameterType.OpenGlInitParams,
                    Data = (void*)initParamsPtr,
                },
            ...
            fixed (MpvRenderParameter* parameterPtr = parameters)
            {
                lock (_gate)
                {
                    ...
                    ThrowIfError(
                        MpvRenderApi.CreateContext(&createdContext, _handle, parameterPtr),
                        nameof(MpvRenderApi.CreateContext)
                    );
    ...
    )
    {
        ...
        [
            ...
            new MpvRenderParameter
            {
                Type = MpvRenderParameterType.BlockForTargetTime,
                Data = &blockForTargetTime,
            },
            new MpvRenderParameter { Type = MpvRenderParameterType.Invalid, Data = null },
        ];
        ...
        fixed (MpvRenderParameter* parameterPtr = parameters)
        {
            lock (_gate)
            {
                ...
                ThrowIfError(
                    MpvRenderApi.Render(_renderContext, parameterPtr),
                    nameof(MpvRenderApi.Render)
                );
    ...
    private static unsafe MpvHandle* CreateHandle(MpvClientOptions options)
    {
        if (options.SharedClient == null)
            return MpvNativeApi.Create();

        return options.UseWeakReference
            ? MpvNativeApi.CreateWeakClient(options.SharedClient._handle, options.SharedClientName)
            : MpvNativeApi.CreateClient(options.SharedClient._handle, options.SharedClientName);
    }
    ...
    {
        if (_renderContext == null || _options.SharedClient != null)
            return;
    ...
    private void CompleteAsyncRequest(MpvEvent mpvEvent)
    {
        ...
        if ((int)mpvEvent.Error < 0)
        {
            completion.TrySetException(
                new AesMpvException(mpvEvent.Error, MpvNativeApi.DescribeError((int)mpvEvent.Error))
            );
            return;
        }
    ...
    private void ThrowIfError(int errorCode, string function, params string[] args)
    {
        ...
        var message =
            $"{function}({string.Join(",", args)}) error: {MpvNativeApi.DescribeError(errorCode)}";
    ...
    private void EnsureActiveHandle()
    {
        if (_handle == null || _disposed)
            throw new AesMpvException(
                MpvError.Uninitialized,
                "The AES mpv player handle is null or disposed."
            );
    }
