...
internal abstract class CombinedGenerator : ISourceGenerator
{
    ...
    private List<StaticGenerator> StaticFileGenerators { get; } = new();

    public void Initialize(GeneratorInitializationContext context)
    {
        var files = StaticFileGenerators.SelectMany(x => x.Generate()).ToArray();
    ...
    protected void AddStep<T>()
        where T : GeneratorStep, new()
    {
        StepFactories.Add(() => new T());
    }

    protected void AddStatic<T>()
        where T : StaticGenerator, new()
    {
